...
import { CHECK_ITEMS, LEGACY_CHECK_KEY } from '../config.js';
...
export const CHECK_LABEL = (id) => {
  ...
  if (item) return TX(item.key);
  if (LEGACY_CHECK_KEY[id]) return TX(LEGACY_CHECK_KEY[id]); // e.g. old combined 'clean_oil'
  return id;
};
